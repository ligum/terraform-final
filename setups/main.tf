...
# #Create a network interface for private/db vm.
...
#Create a network_security_group for private subnet 
resource "azurerm_network_security_group" "nsg_private" {
  ...
  resource_group_name = azurerm_resource_group.rg.name
    ...
    destination_port_range     = "22"
    source_address_prefix      = azurerm_public_ip.publicipforvm1.ip_address
    destination_address_prefix = azurerm_public_ip.publicipforvm1.ip_address
  }
    security_rule {
    name                       = "Port_5432"
    priority                   = 101
    ...
    source_port_range          = "22"
    destination_port_range     = "22"
    source_address_prefix      = azurerm_public_ip.publicipforvm2.ip_address
    destination_address_prefix = azurerm_public_ip.publicipforvm2.ip_address
  }
    security_rule {
    name                       = "Port_5432_VM2"
    priority                   = 103
...
# # # #Create a association between nsg and sbnet (private)
...
resource "azurerm_network_interface_security_group_association" "nsg_nicdb" {
  network_interface_id      = azurerm_network_interface.network_interface_private.id
  network_security_group_id = azurerm_network_security_group.nsg_private.id
}
...
module "vmdb" {
  ...
  VIRTUAL_MACHINE_NAME  = "bootcampAppVMDB"
  RESOURCE_GROUP_NAME   = azurerm_resource_group.rg.name
  AZURE_LOCATION        = azurerm_resource_group.rg.location
  AVAILABILITY_SET_ID   = "" 
  STUDENT_NAME          = "Vova"
  VM_SIZE               = "Standard_B1s"
  NETWORK_INTERFACE_IDS = azurerm_network_interface.network_interface_private.id
  STORAGE_OS_DISK_NAME  = "bootcamp-storage-os-diskdb"
  VM_ADMIN_PASSWORD     = var.VM_ADMIN_PASSWORD_DB
}
